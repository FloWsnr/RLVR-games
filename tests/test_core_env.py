...
from rlvr_games.games.chess import ChessEnv, STANDARD_START_FEN
...
class CounterScenario:
    def reset(self, *, seed: int | None = None) -> tuple[CounterState, dict[str, Any]]:
...
class TurnBasedEnvTests(unittest.TestCase):
    ...
    def test_records_trajectory_until_terminal(self) -> None:
        env = TurnBasedEnv(
            ...
            renderer=CounterRenderer(),
            reward_fn=CounterReward(),
            config=EpisodeConfig(max_turns=10, seed=7),
        )
